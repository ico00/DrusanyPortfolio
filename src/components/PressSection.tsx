...
export default function PressSection({ items }: PressSectionProps) {
  ...
  const openLightbox = useCallback((item: PressItem) => {
  ...
  const columns = useMemo(() => {
    const cols: PressItem[][] = Array.from({ length: columnCount }, () => []);
  ...
  return (
    <section className="mt-16 border-t border-zinc-800 pt-16">
      ...
      <div className="prose prose-invert prose-lg max-w-xl mb-10 [&_p]:mb-0">
        <p className="text-zinc-400">Published here, published there...</p>
      </div>
      <div
        ...
        {lightboxItem && (
          ...
          >
            ...
            >
              ...
              {(lightboxItem.caption || lightboxItem.link) && (
              <div
                  ...
                  <div className="inline-block rounded-lg border border-white/10 bg-black/50 px-4 py-3 backdrop-blur-sm">
                    ...
                    {lightboxItem.link && (
                      <a
                        ...
                        onClick={(e) => e.stopPropagation()}
                        className={`inline-flex items-center gap-2 text-sm text-amber-400 hover:text-amber-300 transition-colors ${lightboxItem.caption ? "mt-2" : ""}`}
                      >
