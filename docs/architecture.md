...
**Next.js Metadata:**
- `generateMetadata({ params })` u `src/app/(with-footer)/blog/[slug]/page.tsx` – čita blog.json, pronalazi post po slug-u, vraća title i description za `<head>`
- **Open Graph po kategoriji:** `src/app/(with-footer)/blog/kategorija/[slug]/page.tsx` – `og:image` / Twitter card = apsolutni URL thumbnaila najnovijeg posta u toj kategoriji (za dijeljenje linka `/blog/kategorija/...` na društvenim mrežama)
...
- **Canonical URL:** Generira se iz `NEXT_PUBLIC_SITE_URL` + path (npr. `https://drusany.com/blog/251228-advent-2025`). Čisti URL-ovi (bez `.html`); Apache `.htaccess` fallback servira odgovarajući `.html`. Svaka stranica postavlja `alternates.canonical` s relativnom putanjom; Next.js koristi `metadataBase` za rezoluciju u puni URL.

**Sitemap i robots.txt:**
- **sitemap.ts** (`src/app/sitemap.ts`) – programatski generira `sitemap.xml` pri buildu; uključuje: statične stranice (/ , /about, /contact, /blog), blog paginaciju (/blog/page/2, …), **sve kategorijske liste** (`/blog/kategorija/{slug}` iz `getAllBlogCategorySlugs()`), sve blog postove (/blog/[slug]); `lastModified` za postove iz datuma objave; `changeFrequency` i `priority` po tipu stranice; koristi `NEXT_PUBLIC_SITE_URL` (fallback: https://drusany.com)
...
- **Metadata u jednom redu:** Datum, vrijeme (`type="time"`), Status, **Autor teksta**, **Autor fotografija** (placeholder "Ivica Drusany") i Featured post (`Star`) u zajedničkom `flex` kontejneru (`flex-wrap`, `items-end`, `gap-x-4`); na užim širinama prelom u više redaka
...
- **Nova polja u form state:** `openCreate` / `openEdit` moraju postavljati sva polja iz `BlogPost` (npr. `authorText`, `authorPhoto`) – inače TypeScript pada na `SetStateAction`
...
- **contentReady:** Formatting toolbar i File panel montiraju se tek kad je `contentReady` true – postavlja se nakon što `replaceBlocks` završi učitavanje HTML-a (`setTimeout(0)`), da se izbjegne race s Floating UI-jem (`Cannot read properties of undefined (reading 'isConnected')`)
...
- **POST:** Prima novi post; slug se generira putem `normalizeBlogSlug` (format `yymmdd-naslov`); polja **`authorText`** / **`authorPhoto`** (opcionalno) spremaju se u `blog.json`
- **PUT:** Ažuriranje posta (uključujući `authorText` / `authorPhoto`); pri promjeni slug-a/datuma – `blogCleanup` automatski: premješta uploads folder, ažurira putanje u blog.json (thumbnail, gallery, galleryMetadata), blogExif.json, **HTML sadržaju** (img src, data-url); briše stari `[slug].html` kad se slug mijenja. **Admin:** promjena datuma u DatePickeru automatski ažurira slug (yymmdd-naslov). **Validacija slika** – ako novi body ima manje slika nego stari, vraća **409** (`error: "images_removed"`, `removedCount`, `removedUrls`); klijent prikazuje confirm; retry s `forceSave: true` za namjerno spremanje
...
│   │   ├── (with-footer)/
│   │   │   └── blog/
│   │   │       ├── [[...segments]]/page.tsx   # /blog, /blog/page/N
│   │   │       ├── kategorija/[slug]/page.tsx # Lista po kategoriji + OG
│   │   │       └── [slug]/page.tsx            # Pojedinačni post
│   │   ├── contact/page.tsx
...
│   │   ├── sitemap.ts      # Generira sitemap.xml (stranice, blog, paginacija, /blog/kategorija/*)
│   │   └── globals.css
│   ├── components/
│   │   ├── blog/
...
│   │   │   ├── BlogListLayout.tsx    # Layout liste; opcionalno initialCategorySlug
│   │   │   ├── Pagination.tsx        # Paginacija; categorySlug za /blog/kategorija vs ?kategorija=
│   │   │   ├── SearchWidget.tsx      # Filter-as-you-type; router.replace na puni pathname (ostaje /blog/kategorija/... s ?q=)
...
│       ├── blogCategories.ts  # BLOG_CATEGORIES, getBlogCategoryOptions, getBlogCategoryStackedChartData, postHasCategory, blogCategoryListPath, getAllBlogCategorySlugs, getCategorySlugFromBlogPathname
...
- **Blog (`/blog`, `/blog/page/N`, `/blog/kategorija/[slug]`):** Lista postova – **bez naslova** na vrhu (previše praznog prostora na mobilu); kartice s naslovom, metapodacima (autor teksta/fotografija iz `authorText`/`authorPhoto`, Datum objave, Kategorija) i slikom ispod; bez overlayja na slikama; **BlogSidebar** s desne strane – **SearchWidget** (debounced 300ms; **text-base** 16px – sprječava iOS Safari zoom pri fokusu; pretraga po naslovu, slug-u, kategorijama, sadržaju; **`router.replace` na puni pathname** da pretraga na `/blog/kategorija/...` ne skoči na `/blog`), **CategoriesWidget** (kategorije abecedno; **accordion** za Sport, Gradovi – chevron na desnoj strani s rotacijom, podkategorije abecedno sortirane), **FeaturedPostsWidget** (istaknuti članci, **max 3** – validacija u AdminBlog i API, toast pri prekoračenju), **PlansWidget** (planirani snimanja – datum + naziv iz plans.json, ispod Istaknutih članaka), GoogleMapsWidget (embed iz Google My Maps); **pretraga** po `q` parametru; sortiranje od najnovijeg; **filtiranje kategorija** – `?kategorija=` ili ruta **`/blog/kategorija/slug`** (preporučeno za dijeljenje zbog OG); glatka fade animacija (AnimatePresence); `scroll={false}` na linkovima da stranica ne skrola na vrh; **Suspense** – jedan `Suspense` oko cijelog sadržaja (SearchWidget + BlogList + BlogSidebar) radi izbjegavanja hydration mismatch s `useSearchParams()`; `/blog/[slug]` – pojedinačni post: naslov i metapodaci na vrhu (**poravnani s sidebarom** – bez pt-5), featured slika ispod, prose body (slike s poravnanjem, vizual kao galerija; **YouTube video blok** – embed po širini stranice; **overflow-x-hidden** na content wrapperu sprječava preljev slika), galerija na dnu (EXIF iz blogExif.json); bijela pozadina; **Footer** na dnu stranice; **ScrollToTop** gumb – pozicioniran desno od ruba sadržaja članka (`lg:right-[max(27rem,calc(50vw-14.5rem))]`)
- **Blog metapodaci:** Ikone (PenLine, Camera, Calendar, Tag); autor teksta i fotografija iz `authorText`/`authorPhoto` (default: "Ivica Drusany") – kad su isti: „Tekst i fotografije: X“, kad su različiti: odvojeno „Tekst: X“ i „Fotografije: Y“; „Datum objave: dd. mm. yyyy.“, „Kategorija:“ s linkovima na **`/blog/kategorija/{slug}`** (donja crta, border-b); razmak između blokova: inline `marginRight: "3rem"` (Safari kompatibilnost). **Mobilna verzija:** autor ispod featured slike – kompaktno (kad su isti) ili dva reda (kad su različiti); kategorija kao link na **`/blog/kategorija/{slug}`**; search widget isti razmak kao na blog listi (`py-24`)
