...
- **Admin panel** – Samo u development modu: **Dashboard** (kartice: Portfolio, Blog, Portfolio Categories, Blog Categories, Static pages, Blog posts; bar charti "Images by category in portfolio" i "Images by category in blog"; Content health); galerija (upload, edit, hero, sortiranje) s **custom DateTimePicker**; About/Contact (quote, Formspree endpoint); Blog s **DatePicker**, **metadata u jednom redu** (datum, vrijeme, status, featured post), **upload slika u sadržaj** (BlockNote `/image` – **BlogUploadTab**: drag-and-drop + file input, **Media** tab, Embed), **resize** slika; **BlockNote** – **Formatting toolbar na vrhu bloka** i uz **prazan kursor** u tekstualnom bloku; tip bloka (npr. slika, YouTube) preko slash menija `/`; tooltipovi gumba (Bold, …) – tamna tema u `globals.css`; **Media** – agregirani prikaz slika, filter, search, paginacija, lightbox, bulk akcije; **Blog widgets** (`/admin?tab=widgets`) – redoslijed sidebara, naslovi, maps embedi, **planovi** (`plans.json` + Save plans), grid poravnanje Title stupca; **Theme** – fontovi, boje, live preview; **sidebar accordion**; **toast**
...
│   │   ├── api/          # upload, update, delete, reorder, hero, gallery, gallery/generate-slugs, media, media-delete, media-detach, content-health, health, pages, blog, blog-upload, blog-delete-file, blog-widgets, plans, theme
...
│   ├── components/       # … BlogWidgetsAdmin, blocknote/ (BlogFilePanel, BlogUploadTab, …), BlockTopFormattingToolbarController, AdminBlog, …
...
- **Blog widgets:** `/admin?tab=widgets` – redoslijed widgeta u sidebaru (dnd-kit), naslovi, Google Maps embedi, **planovi** (`plans.json`); paralelni učitavanje widgeta i planova; **Save widgets** / **Save plans**; `PUT /api/blog-widgets`, `PUT /api/plans`; UI konstante `ADMIN_UI.adminWidgets` (fuchsia / LayoutGrid)
